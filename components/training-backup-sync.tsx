...
export function TrainingBackupSync() {
  const { ready, oneRmFormula, plateStepKg, personalRecords, restoreTrainingBackup } = useWorkoutStore();
  ...
  useEffect(() => {
    if (!ready || backupQuery.isLoading || !restored.current) return;
    const snapshotJson = JSON.stringify({ oneRmFormula, plateStepKg, personalRecords });
  ...
  }, [ready, backupQuery.isLoading, oneRmFormula, plateStepKg, personalRecords, saveMutation]);
