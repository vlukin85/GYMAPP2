...
import { pickAndPersistUserImage } from "@/lib/user-media";
...
export function MediaPickerSheet({ visible, title, ownerId, scope, currentImage, library, onSelect, onClose }: MediaPickerSheetProps) {
  ...
  const chooseUpload = async () => {
    try {
      setPicking(true);
      const uri = await pickAndPersistUserImage(scope, ownerId);
      if (uri) { onSelect(uri); onClose(); }
