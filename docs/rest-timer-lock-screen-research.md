...
`NotificationCompat.Builder.setChronometerCountDown(true)` включает убывающий хронометр в Android-уведомлении, если перед этим установлен `setUsesChronometer(true)`. Метод требует Android API 24 или выше. IronRise передаёт окончание отдыха через `setWhen(restEndAt)`: System UI самостоятельно ведёт счётчик, поэтому он продолжает идти, пока JavaScript-процесс приложения неактивен.
...
При отмене будильника используется `PendingIntent.FLAG_NO_CREATE`, который по контракту Android возвращает `null`, если соответствующий PendingIntent ещё не существует. Поэтому очистка таймера должна вызывать `AlarmManager.cancel` только при наличии результата, иначе первоначальная публикация уведомления может аварийно завершиться.

## Источники

- https://developer.android.com/reference/androidx/core/app/NotificationCompat.Builder#setChronometerCountDown(boolean)
- https://developer.android.com/reference/android/app/PendingIntent#FLAG_NO_CREATE
