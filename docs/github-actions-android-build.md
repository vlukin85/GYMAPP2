...
По умолчанию debug APK собирается только для `arm64-v8a` — это современные Android-смартфоны, и такая сборка заметно меньше универсальной, поэтому устанавливается быстрее. Если телефон старый или не поддерживает 64-битные приложения, в поле **Архитектура debug APK** выберите `universal`; он содержит и `arm64-v8a`, и `armeabi-v7a`, но устанавливается дольше.
...
2. Распакуйте ZIP полностью и выберите `app-debug.apk`; у прежней универсальной сборки от 21 августа 2026 года размер файла — около **109 МБ**, SHA-256: `9f7dacc403258e7db9ec453b4ee6c9b4c823de1277c97a892dce89e26cc87fdb`. Новая ARM64-сборка меньше и является рекомендуемым вариантом для современных устройств.
