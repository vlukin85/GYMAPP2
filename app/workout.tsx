...
export default function WorkoutScreen() {
  ...
  const remoteHistory = useMemo(() => {
    ...
    const grouped = new Map<number, { date: string; sets: { weight: number; reps: number }[]; volume: number }>();
    rows.forEach((row) => {
      const existing = grouped.get(row.sessionId) ?? { date: new Date(row.date).toLocaleDateString("ru-RU", { day: "numeric", month: "short" }).replace(".", ""), sets: [], volume: 0 };
  ...
  const historyEntries = remoteHistory.length > 0 ? remoteHistory : (activeExerciseId ? getExerciseHistory(activeExerciseId) : []);
