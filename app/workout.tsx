...
import { bestOneRepMax, calculateVolume, getExercise, getExerciseHistory, getProgram } from "@/lib/workout-data";
...
export default function WorkoutScreen() {
  ...
  const saveWorkoutMutation = trpc.workoutHistory.save.useMutation();
  ...
  const remoteHistoryQuery = trpc.workoutHistory.byExercise.useQuery({ exerciseId: activeExerciseId ?? "" }, { enabled: !!activeExerciseId });
  const remoteHistory = useMemo(() => {
    const rows = remoteHistoryQuery.data ?? [];
    const grouped = new Map<number, { date: string; sets: { weight: number; reps: number }[]; volume: number; bestOneRepMax: number }>();
    rows.forEach((row) => {
      const existing = grouped.get(row.sessionId) ?? { date: new Date(row.date).toLocaleDateString("ru-RU", { day: "numeric", month: "short" }).replace(".", ""), sets: [], volume: 0, bestOneRepMax: 0 };
      existing.sets.push({ weight: row.weightCentiKg / 100, reps: row.reps });
      existing.volume += row.volumeCentiKg / 100;
      existing.bestOneRepMax = Math.max(existing.bestOneRepMax, row.oneRepMaxCentiKg / 100);
      grouped.set(row.sessionId, existing);
    });
    return Array.from(grouped.values());
  }, [remoteHistoryQuery.data]);
  const historyEntries = useMemo(() => { const source = remoteHistory.length > 0 ? remoteHistory : (activeExerciseId ? getExerciseHistory(activeExerciseId) : []); return source.map((entry) => ({ ...entry, bestOneRepMax: entry.bestOneRepMax ?? bestOneRepMax(entry.sets) })); }, [remoteHistory, activeExerciseId]);
  ...
  return <ScreenContainer edges={["top", "left", "right", "bottom"]} className="px-5" containerClassName="bg-background">
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
      ...
      <Pressable disabled={saveWorkoutMutation.isPending} onPress={async () => { if (completed < program.exercises.length) { Alert.alert("Не всё заполнено", "Открой каждое упражнение и сохрани фактические подходы перед завершением тренировки.", [{ text: "Понятно" }]); return; } const sets = program.exercises.flatMap((item) => (actualSets[item.exerciseId] ?? []).map((set, index) => ({ exerciseId: item.exerciseId, setNumber: index + 1, reps: Number(set.reps) || 0, weightKg: Number(set.weight) || 0 }))); try { await saveWorkoutMutation.mutateAsync({ programId: program.id, durationMinutes: Math.max(1, Math.ceil(elapsed / 60)), sets }); finishWorkout(program.id, total); router.replace("/(tabs)"); } catch { Alert.alert("Не удалось сохранить", "Проверь подключение к серверу. Тренировка пока не завершена.", [{ text: "Понятно" }]); } }} style={[styles.finish, { backgroundColor: colors.primary, opacity: saveWorkoutMutation.isPending ? 0.6 : 1 }]}><Text style={styles.finishText}>{saveWorkoutMutation.isPending ? "Сохраняем…" : "Завершить тренировку"}</Text><IconSymbol name="checkmark" size={20} color="#101412" /></Pressable>
    </ScrollView>
    ...
    <Modal visible={!!activeExerciseId} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setActiveExerciseId(null)}>
      <KeyboardAvoidingView style={[styles.modalRoot, { backgroundColor: colors.background }]} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        ...
        <ScrollView contentContainerStyle={styles.modalContent} keyboardShouldPersistTaps="handled">
          ...
          <View style={[styles.oneRmCard, { backgroundColor: colors.primary }]}><View><Text style={styles.oneRmLabel}>ПРЕДПОЛАГАЕМЫЙ 1RM</Text><Text style={styles.oneRmHint}>Оценка по формуле Эпли</Text></View><Text style={styles.oneRmValue}>{bestOneRepMax(draftSets.map((set) => ({ weight: Number(set.weight) || 0, reps: Number(set.reps) || 0 }))).toFixed(1)} кг</Text></View>
          ...
          {historyEntries.length > 0 ? <View style={[styles.historyCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>{historyEntries.map((entry) => { const isExpanded = expandedHistory === entry.date; return <View key={entry.date} style={[styles.historyEntry, { borderBottomColor: colors.border }]}><Pressable onPress={() => setExpandedHistory(isExpanded ? null : entry.date)} style={styles.historyRow}><View style={styles.historyDate}><Text style={[styles.historyDateText, { color: colors.foreground }]}>{entry.date}</Text><Text style={[styles.historyVolume, { color: colors.muted }]}>{entry.volume.toLocaleString("ru-RU")} кг · 1RM {entry.bestOneRepMax.toFixed(1)} кг</Text></View><View style={styles.historySets}><Text style={[styles.historyPreview, { color: colors.foreground }]}>{entry.sets[0]?.weight} кг × {entry.sets[0]?.reps} повт. {entry.sets.length > 1 ? `+ ещё ${entry.sets.length - 1}` : ""}</Text></View><IconSymbol name={isExpanded ? "chevron.down" : "chevron.right"} size={18} color={colors.muted} /></Pressable>{isExpanded && <View style={[styles.historyDetails, { borderTopColor: colors.border }]}><View style={styles.detailHeader}><Text style={[styles.detailHeaderText, styles.detailIndex, { color: colors.muted }]}>ПОДХОД</Text><Text style={[styles.detailHeaderText, { color: colors.muted }]}>ПОВТОРЫ</Text><Text style={[styles.detailHeaderText, { color: colors.muted }]}>ВЕС</Text></View>{entry.sets.map((set, setIndex) => <View key={`${entry.date}-detail-${setIndex}`} style={styles.detailRow}><Text style={[styles.detailValue, styles.detailIndex, { color: colors.muted }]}>{setIndex + 1}</Text><Text style={[styles.detailValue, { color: colors.foreground }]}>{set.reps}</Text><Text style={[styles.detailValue, { color: colors.foreground }]}>{set.weight} кг</Text></View>)}<View style={styles.detailFooter}><Text style={[styles.detailFooterLabel, { color: colors.muted }]}>Объём · 1RM</Text><Text style={[styles.detailFooterValue, { color: colors.primary }]}>{entry.volume.toLocaleString("ru-RU")} кг · {entry.bestOneRepMax.toFixed(1)} кг</Text></View></View>}</View>; })}</View> : <View style={[styles.historyEmpty, { backgroundColor: colors.surface }]}><Text style={[styles.historyEmptyText, { color: colors.muted }]}>Это первое зафиксированное выполнение</Text></View>}
