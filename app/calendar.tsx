...
import { getMonthCalendarDays, isFutureScheduleDate, isScheduledWorkoutCompleted, shiftCalendarMonth } from "@/lib/workout-data";
...
export default function CalendarScreen() {
  ...
  const selectedScheduleCompleted = Boolean(activeSchedule && isScheduledWorkoutCompleted(completed, selectedDate, activeSchedule.programId));
  ...
  const selectDate = (date: Date) => {
    ...
    setSelectedDate(key); setProgramId(nextProgramId); setDraftProgramId(nextProgramId);
    setTime(entry?.time ?? "18:30"); setReminderMinutes(entry?.reminderMinutes ?? 60);
  };

  const openPicker = () => {
    const nextProgramId = programId || programs[0]?.id || "";
    setDraftProgramId(nextProgramId); setPickerOpen(true);
  };
  ...
  const persistPlan = async (chosenProgramId: string) => {
    ...
    try {
      ...
      setProgramId(chosenProgramId); setPickerOpen(false);
      Alert.alert("Тренировка запланирована", `${program.name} · ${readableDate(selectedDate)} в ${time}`);
    } catch { Alert.alert("Не удалось запланировать", "Проверь разрешение на уведомления."); }
  };
  ...
  const beginDrag = (sourceDate: string) => {
    ...
    if (!schedule) return;
    if (isScheduledWorkoutCompleted(completed, sourceDate, schedule.programId)) { Alert.alert("Тренировка уже выполнена", "Выполненную тренировку нельзя переносить. Создайте новую запись в расписании."); return; }
    gridRef.current?.measure((_x, _y, _width, _height, _pageX, pageY) => { gridPageYRef.current = pageY; });
    draggedDateRef.current = sourceDate; dropDateRef.current = null;
    isDraggingRef.current = true; setDraggedDate(sourceDate); setDropDate(null); setDragging(true);
  };
  ...
  const updateDropTarget = (pageX: number, pageY: number) => {
    ...
    if (date) { const target = dateKey(date); dropDateRef.current = target; setDropDate(target); }
  };

  const finishDrag = async () => {
    const from = draggedDateRef.current;
    const to = dropDateRef.current;
    draggedDateRef.current = null; dropDateRef.current = null;
    isDraggingRef.current = false; setDragging(false); setDraggedDate(null); setDropDate(null);
    if (!from || !to || from === to) return;
    const schedule = scheduled[from];
    if (!schedule) return;
    if (scheduled[to]) { Alert.alert("Дата занята", "Выберите свободный день для переноса тренировки."); return; }
    ...
    try {
      ...
      scheduleProgram(to, { ...schedule, notificationId }); removeSchedule(from);
      setSelectedDate(to); setProgramId(schedule.programId); setDraftProgramId(schedule.programId); setTime(schedule.time); setReminderMinutes(schedule.reminderMinutes);
      Alert.alert("Тренировка перенесена", `${program?.name ?? "Тренировка"} перенесена на ${readableDate(to)}.`);
    } catch { Alert.alert("Не удалось перенести", "Проверь подключение и разрешения для уведомлений."); }
  };
