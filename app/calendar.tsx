...
import { getMonthCalendarDays, isFutureScheduleDate } from "@/lib/workout-data";
...
export default function CalendarScreen() {
  ...
  const beginDrag = (sourceDate: string) => {
    if (!scheduled[sourceDate]) return;
    gridRef.current?.measure((_x, _y, _width, _height, _pageX, pageY) => setGridPageY(pageY));
    draggedDateRef.current = sourceDate; dropDateRef.current = null;
    setDraggedDate(sourceDate); setDropDate(null); setDragging(true);
  };

  const updateDropTarget = (pageX: number, pageY: number) => {
    if (!isDragging || !draggedDateRef.current) return;
    const cellWidth = (Dimensions.get("window").width - 40) / 7;
    const col = Math.max(0, Math.min(6, Math.floor((pageX - 20) / cellWidth)));
    const row = Math.max(0, Math.min(5, Math.floor((pageY - gridPageY) / 47)));
  ...
  const finishDrag = async () => {
    ...
    setDragging(false); setDraggedDate(null); setDropDate(null);
