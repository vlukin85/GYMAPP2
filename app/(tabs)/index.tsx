...
import { buildHomeEnergyTrend, type HomeEnergyTrendPoint } from "@/lib/home-energy-trend";
...
export default function HomeScreen() {
  ...
  const { profile, heightCm, ageYears, measurements, activityLevel } = useBodyStore();
  ...
  const dailyEnergy = calculateDailyEnergy({ profile, weightKg: currentWeightKg, heightCm, ageYears, activityLevel, workoutCalories: workoutCaloriesToday });
  const weeklyEnergyTrend = buildHomeEnergyTrend(now, completed, { profile, weightKg: currentWeightKg, heightCm, ageYears, activityLevel });
  ...
  return (
    <ScreenContainer className="px-0" containerClassName="bg-background">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <HomeWidgetStack homeWidgets={homeWidgets} now={now} scheduled={scheduled} completed={completed} colors={colors} foodCalories={foodCalories} dailyCalorieGoal={dailyCalorieGoal} foodMacros={foodMacros} dailyMacroGoals={dailyMacroGoals} workoutTrend={workoutTrend} weekVolume={weekVolume} dailyQuote={dailyQuote} dailyEnergy={dailyEnergy} weeklyEnergyTrend={weeklyEnergyTrend} />
      </ScrollView>
    </ScreenContainer>
  );
}

function HomeWidgetStack({ homeWidgets, now, scheduled, completed, colors, foodCalories, dailyCalorieGoal, foodMacros, dailyMacroGoals, workoutTrend, weekVolume, dailyQuote, dailyEnergy, weeklyEnergyTrend }: any) {
...
function WeeklyEnergyTrend({ points, colors, compact }: { points: HomeEnergyTrendPoint[]; colors: any; compact: boolean }) {
  const average = Math.round(points.reduce((sum, point) => sum + point.totalCalories, 0) / Math.max(1, points.length));
  const width = 308;
  const height = compact ? 74 : 112;
  const baseline = compact ? 54 : 82;
  const maxCalories = Math.max(1, ...points.map((point) => point.totalCalories));
  const gap = 10;
  const barWidth = Math.max(18, Math.min(28, (width - 24 - gap * Math.max(0, points.length - 1)) / Math.max(1, points.length)));
  return <Pressable onPress={() => router.push("/(tabs)/body")} style={({ pressed }) => [styles.energyTrend, compact && compactStyles.energyTrend, { borderColor: colors.border, backgroundColor: colors.surface, opacity: pressed ? 0.68 : 1 }]}><View style={styles.energyTrendHead}><View><Text style={[styles.energyKicker, { color: colors.primary }]}>РАСХОД · 7 ДНЕЙ</Text><Text style={[styles.energyTrendValue, { color: colors.foreground }]}>{average.toLocaleString("ru-RU")} <Text style={styles.energySuffix}>ККАЛ В СРЕДНЕМ</Text></Text></View><IconSymbol name="chart.bar.fill" size={compact ? 21 : 25} color={colors.primary} /></View><Svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`}><Line x1={12} x2={width - 12} y1={baseline} y2={baseline} stroke={colors.border} strokeWidth="1" />{points.map((point, index) => { const x = 12 + index * (barWidth + gap); const barHeight = Math.max(6, (point.totalCalories / maxCalories) * (compact ? 44 : 65)); return <Rect key={point.date} x={x} y={baseline - barHeight} width={barWidth} height={barHeight} fill={point.workoutCalories > 0 ? colors.primary : colors.foreground} />; })}</Svg><View style={styles.chartAxis}>{points.map((point) => <Text key={point.date} style={[styles.axisLabel, { color: colors.muted }]}>{point.label}</Text>)}</View><Text style={[styles.energyTrendHint, { color: colors.muted }]}>Акцент — дни с завершённой тренировкой.</Text></Pressable>;
}
...
const styles = StyleSheet.create({
  ...
  energyTrend: { marginHorizontal: 12, marginTop: 12, borderWidth: 1, padding: 14 },
  energyTrendHead: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start" },
  energyTrendValue: { fontSize: 17, fontWeight: "900", letterSpacing: -0.5, marginTop: 3 },
