...
import {
  buildHomeEnergyTrend,
  type HomeEnergyTrendPoint,
} from "@/lib/home-energy-trend";
...
export default function HomeScreen() {
  ...
  const { programs, completed, scheduled, startWorkout, bodyWeightKg } =
    useWorkoutStore();
  const { profile, heightCm, ageYears, measurements, activityLevel } =
    useBodyStore();
  const {
  ...
  const todayProgram = todaySchedule
    ? (getProgram(todaySchedule.programId) ??
      programs.find((program) => program.id === todaySchedule.programId))
    : undefined;
  const workoutTrend = buildHomeWorkoutTrend(completed);
  const weekVolume = completed.reduce((sum, item) => sum + item.totalVolume, 0);
  const dayLabel = titleCase(
    now.toLocaleDateString("ru-RU", { weekday: "long" }),
  );
  const monthLabel = now
    .toLocaleDateString("ru-RU", { month: "long" })
    .toUpperCase();
  const todayFoodEntries = nutritionEntries.filter(
    (entry) => entry.date === nutritionToday,
  );
  const foodCalories = todayFoodEntries.reduce(
    (sum, entry) => sum + entryCalories(entry),
    0,
  );
  const foodMacros = sumEntryMacros(todayFoodEntries);
  const dailyQuote = getDailyAthleteQuote(now);
  const latestRecordedWeight = measurements.find(
    (measurement) =>
      typeof measurement.weightKg === "number" && measurement.weightKg > 0,
  )?.weightKg;
  ...
  const workoutCaloriesToday = completed
    ...
    .reduce(
      (sum, workout) => sum + Math.max(0, workout.caloriesBurned ?? 0),
      0,
    );
  ...
  const weeklyEnergyTrend = buildHomeEnergyTrend(now, completed, {
    profile,
    weightKg: currentWeightKg,
    heightCm,
    ageYears,
      ...
      >
        ...
        <View style={styles.heroGrid}>
          <View style={[styles.heroLeft, { borderColor: colors.border }]}>
            <Text style={[styles.sideMotto, { color: colors.foreground }]}>
              ПОДНИМАЙ. ПРОГРЕССИРУЙ. ПОВТОРЯЙ.
            </Text>
            <View
              style={[styles.mottoRule, { backgroundColor: colors.foreground }]}
            />
          </View>
          <View style={[styles.dayPanel, { borderColor: colors.border }]}>
            ...
            <View style={styles.dayMeta}>
              <Text style={[styles.redKicker, { color: colors.primary }]}>
                СЕГОДНЯ
              </Text>
              ...
              <View
                style={[styles.redRule, { backgroundColor: colors.primary }]}
              />
            </View>
            <Text
              numberOfLines={1}
            ...
            >
              IRON<Text style={{ color: colors.primary }}>RISE</Text>
            </Text>
            <Text style={[styles.tagline, { color: colors.foreground }]}>
              СТАНЬ СИЛЬНЕЕ СЕБЯ ВЧЕРАШНЕГО.
            </Text>
            <View style={styles.brandCredit}>
              <Text style={[styles.creditName, { color: colors.foreground }]}>
                by Vasily Lukin
              </Text>
              <Text style={[styles.creditRights, { color: colors.muted }]}>
                © All rights reserved
              </Text>
            </View>
            <View
              style={[styles.blueBlock, { backgroundColor: REFERENCE_BLUE }]}
            />
          </View>
          <View style={[styles.planPanel, { borderColor: colors.border }]}>
            <View
              style={[styles.planHeading, { borderBottomColor: colors.border }]}
            >
              <Text style={[styles.planStar, { color: colors.primary }]}>
                ✳
              </Text>
            ...
            {todayProgram ? (
              <PlanTimeline program={todayProgram} colors={colors} />
            ) : (
              <EmptyPlan colors={colors} />
            )}
          </View>
        </View>
        <HomeWidgetStack
          ...
          weeklyEnergyTrend={weeklyEnergyTrend}
        />
      </ScrollView>
    </ScreenContainer>
  );
...
function HomeWidgetStack({
  ...
  const visibleIds = homeWidgets.order.filter(
    (id: HomeWidgetId) => homeWidgets.visibility[id],
  );
  ...
  const finishWidgetDrag = (id: HomeWidgetId, target: number) => {
    ...
    const destination = destinationId
      ? homeWidgets.order.indexOf(destinationId)
      : homeWidgets.order.length - 1;
    animateWidgetLayout();
  ...
  const renderWidget = (id: HomeWidgetId) => {
    ...
    if (id === "quote")
      return (
        <View
          ...
          style={[
            styles.quoteCard,
            compact && compactStyles.quoteCard,
        ...
        >
          ...
          />
          <Text style={[styles.quoteKicker, { color: colors.primary }]}>
            ЦИТАТА ДНЯ
          </Text>
          <Text
            ...
            style={[
              styles.quoteText,
              compact && compactStyles.quoteText,
          ...
          <Text style={[styles.quoteAuthor, { color: colors.muted }]}>
            {dailyQuote.athlete.toUpperCase()} ·{" "}
            {dailyQuote.discipline.toUpperCase()}
          </Text>
        </View>
      );
    if (id === "week")
    ...
    if (id === "trainingTrend")
      return (
        <View
          ...
          style={[
            styles.analytics,
            compact && compactStyles.analytics,
            { borderTopColor: colors.border, borderBottomColor: colors.border },
          ]}
        >
          <View
            style={[styles.analyticsLabel, { borderRightColor: colors.border }]}
          >
            <Text style={[styles.analyticsTitle, { color: colors.foreground }]}>
              ОБЪЁМ{`\n`}КГ
            </Text>
            <View
              style={[styles.graphMark, { backgroundColor: colors.foreground }]}
            >
              <IconSymbol
                name="chart.bar.fill"
                size={compact ? 24 : 34}
          ...
          <View style={styles.chartArea}>
            <View style={styles.chartTopline}>
              <Text style={[styles.chartCaption, { color: colors.foreground }]}>
              ...
              >
                <Text style={[styles.chartLinkText, { color: colors.primary }]}>
                  СТАТИСТИКА
                </Text>
              </Pressable>
            </View>
            <CompletedWorkoutChart
              points={workoutTrend}
              colors={colors}
              compact={compact}
            />
    ...
    if (id === "metrics")
      return (
        <View key={id} style={styles.metricsStack}>
          <DailyEnergyCard
            estimate={dailyEnergy}
            colors={colors}
            compact={compact}
          />
          <WeeklyEnergyTrend
            points={weeklyEnergyTrend}
            colors={colors}
            compact={compact}
          />
          <View style={[styles.dataRow, compact && compactStyles.dataRow]}>
            <Metric
              compact={compact}
              label="ТРЕНИРОВОК"
    ...
    return (
      <View
        key={id}
        style={[styles.footerActions, compact && compactStyles.footerActions]}
      >
        <Pressable
          onPress={() => router.push("/(tabs)/calendar")}
          style={({ pressed }) => [
            styles.outlineAction,
            compact && compactStyles.outlineAction,
  ...
  return (
    <>
      {homeWidgets.ready && !homeWidgets.dragHintSeen && (
        <WidgetDragHint
          colors={colors}
          onDismiss={homeWidgets.dismissWidgetDragHint}
        />
      )}
      {displayedIds.map((id: HomeWidgetId, index: number) => (
        <DraggableHomeWidget
          key={id}
          id={id}
          index={index}
          ...
          colors={colors}
          onDragStart={() => {
            triggerWidgetGrabFeedback();
            setDraggingId(id);
            setDragTarget(index);
            setPreviewOrder(visibleIds);
          }}
          onDragMove={(next) => previewWidgetMove(id, next)}
          onDragEnd={(target) => finishWidgetDrag(id, target)}
        >
          {renderWidget(id)}
        </DraggableHomeWidget>
      ))}
  ...
  return (
    <Modal transparent animationType="fade" visible onRequestClose={onDismiss}>
      <View style={styles.hintBackdrop}>
        ...
        >
          <Text style={[styles.hintEyebrow, { color: colors.primary }]}>
            НАСТРОЙТЕ «СЕГОДНЯ» ПОД СЕБЯ
          </Text>
          <Text style={[styles.hintTitle, { color: colors.foreground }]}>
            Перемещайте виджеты
          </Text>
          <Text style={[styles.hintText, { color: colors.muted }]}>
            Зажмите маркер ⠿ на карточке и потяните её вверх или вниз. Соседние
            блоки плавно освободят место, а новый порядок сохранится
            автоматически.
          </Text>
          <Pressable
            onPress={onDismiss}
            style={({ pressed }) => [
              styles.hintButton,
              { backgroundColor: colors.primary, opacity: pressed ? 0.74 : 1 },
            ]}
          >
...
function DraggableHomeWidget({
  ...
  useEffect(() => {
    Animated.timing(lift, {
      ...
      duration: dragging ? 210 : 260,
      easing: Easing.out(Easing.cubic),
      useNativeDriver: true,
    }).start();
  }, [dragging, lift]);
  const trackedPanResponder = useMemo(
    () =>
      PanResponder.create({
        ...
        onPanResponderGrant: () => {
          currentTargetRef.value = index;
          onDragStart();
        },
        onPanResponderMove: (_event, gesture) => {
          const next = Math.max(
            0,
            Math.min(total - 1, index + Math.round(gesture.dy / 96)),
          );
          currentTargetRef.value = next;
          onDragMove(next);
        },
  ...
  return (
    ...
    >
      ...
      <View
        ...
        accessibilityLabel="Перетащить виджет"
        style={[
          styles.widgetHandle,
          {
            backgroundColor: dragging ? colors.primary : colors.background,
            borderColor: dragging ? colors.primary : colors.border,
          },
        ]}
        {...trackedPanResponder.panHandlers}
      >
        <Text
          style={[
            styles.widgetHandleText,
            { color: dragging ? "#FFFFFF" : colors.muted },
          ]}
        >
          ⠿
        </Text>
...
function EmptyPlan({ colors }: { colors: any }) {
  return (
    <View style={styles.emptyPlan}>
      <Text style={[styles.emptyPlanText, { color: colors.muted }]}>
        На сегодня программа не запланирована.
      </Text>
      <Pressable
        onPress={() => router.push("/(tabs)/calendar")}
        style={({ pressed }) => [
      ...
      >
        <Text style={styles.planButtonText}>ЗАПЛАНИРОВАТЬ</Text>
      </Pressable>
    </View>
  );
}
  ...
  return (
    <View style={styles.timeline}>
      {plan.map((exercise, index) => {
        ...
        return (
          <View
            key={`${program.id}-${exercise.exerciseId}`}
            style={[
              styles.planItem,
              index < plan.length - 1 && {
          ...
          >
            ...
            <View style={styles.planBody}>
              ...
              >
                {getExercise(exercise.exerciseId)?.name ?? "Упражнение"}
              </Text>
              <Text style={[styles.exerciseDetail, { color: colors.muted }]}>
                {detail} · отдых {exercise.rest} с
              </Text>
            </View>
          </View>
        );
      })}
      {program.exercises.length > plan.length && (
        <Text style={[styles.morePlan, { color: colors.muted }]}>
          + ещё {program.exercises.length - plan.length} в программе
        </Text>
      )}
    </View>
  );
...
function WeekStrip({
  ...
  const cells = Array.from({ length: 7 }, (_, index) => {
    ...
    return {
      date,
      key,
      done: completed.some((item) => item.date === key),
      planned: Boolean(scheduled[key]),
    ...
    <View style={[styles.weekStrip, { borderBottomColor: colors.border }]}>
      {cells.map((cell) => (
        <Pressable
          ...
          style={({ pressed }) => [
            styles.weekCell,
            compact && compactStyles.weekCell,
            {
              borderRightColor: colors.border,
              backgroundColor: cell.today ? REFERENCE_BLUE : "transparent",
        ...
        >
          <Text
            style={[
              styles.weekDay,
              { color: cell.today ? colors.surface : colors.foreground },
            ]}
          >
            {cell.date
          ...
          <Text
            style={[
              styles.weekDate,
              compact && compactStyles.weekDate,
              { color: cell.today ? colors.surface : colors.foreground },
            ]}
          >
            {cell.date.getDate()}
...
function Metric({
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        styles.metric,
        compact && compactStyles.metric,
        { borderColor: colors.border, opacity: pressed ? 0.64 : 1 },
      ]}
    >
      <Text
        numberOfLines={1}
        style={[
          styles.metricValue,
          compact && compactStyles.metricValue,
          { color: colors.foreground },
        ]}
      >
        {value}
      ...
      <Text
        style={[
          styles.metricLabel,
          compact && compactStyles.metricLabel,
          { color: colors.muted },
        ]}
      >
        {label}
...
function DailyEnergyCard({
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        styles.energyCard,
        compact && compactStyles.energyCard,
        {
    ...
    >
      <View style={styles.energyHead}>
        <View>
          <Text style={[styles.energyKicker, { color: colors.primary }]}>
            РАСХОД · СЕГОДНЯ
          </Text>
          <Text
            style={[
              styles.energyValue,
              compact && compactStyles.energyValue,
              { color: colors.foreground },
            ]}
          >
            {estimate.totalCalories.toLocaleString("ru-RU")}{" "}
            <Text style={styles.energySuffix}>ККАЛ</Text>
          </Text>
        </View>
        <IconSymbol
          name="bolt.fill"
      ...
      {!compact && (
        <View
          style={[styles.energyBreakdown, { borderTopColor: colors.border }]}
        >
          <Text style={[styles.energyPart, { color: colors.muted }]}>
            Покой{" "}
            <Text
              style={[styles.energyPartValue, { color: colors.foreground }]}
            >
              {estimate.restingCalories}
            </Text>
          </Text>
          <Text style={[styles.energyPart, { color: colors.muted }]}>
            Движение{" "}
            <Text
              style={[styles.energyPartValue, { color: colors.foreground }]}
            >
              {estimate.movementCalories}
            </Text>
          </Text>
          <Text style={[styles.energyPart, { color: colors.muted }]}>
            Тренировка{" "}
            <Text style={[styles.energyPartValue, { color: colors.primary }]}>
              {estimate.workoutCalories}
            </Text>
          </Text>
        </View>
      )}
      <Text style={[styles.energyHint, { color: colors.muted }]}>
        {estimate.isPersonalizedRestingEstimate
          ? "Основано на профиле тела и завершённых тренировках."
          : "Добавьте рост и возраст в профиле для более точной оценки."}
      </Text>
    </Pressable>
  );
}

function WeeklyEnergyTrend({
  points,
  colors,
  compact,
}: {
  points: HomeEnergyTrendPoint[];
  colors: any;
  compact: boolean;
}) {
  const average = Math.round(
    points.reduce((sum, point) => sum + point.totalCalories, 0) /
      Math.max(1, points.length),
  );
  const width = 308;
  const height = compact ? 74 : 112;
  const baseline = compact ? 54 : 82;
  const maxCalories = Math.max(
    1,
    ...points.map((point) => point.totalCalories),
  );
  const gap = 10;
  const barWidth = Math.max(
    ...
    Math.min(
      28,
      (width - 24 - gap * Math.max(0, points.length - 1)) /
        Math.max(1, points.length),
    ),
  );
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        styles.energyTrend,
        compact && compactStyles.energyTrend,
        {
          borderColor: colors.border,
          backgroundColor: colors.surface,
          opacity: pressed ? 0.68 : 1,
    ...
    >
      <View style={styles.energyTrendHead}>
        <View>
          <Text style={[styles.energyKicker, { color: colors.primary }]}>
            РАСХОД · 7 ДНЕЙ
          </Text>
          <Text style={[styles.energyTrendValue, { color: colors.foreground }]}>
            {average.toLocaleString("ru-RU")}{" "}
            <Text style={styles.energySuffix}>ККАЛ В СРЕДНЕМ</Text>
          </Text>
        </View>
        <IconSymbol
          name="chart.bar.fill"
      ...
      <Svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        {points.map((point, index) => {
          ...
          const barHeight = Math.max(
            6,
            (point.totalCalories / maxCalories) * (compact ? 44 : 65),
          );
          return (
            <Rect
              key={point.date}
              ...
              height={barHeight}
              fill={
                point.workoutCalories > 0 ? colors.primary : colors.foreground
              }
            />
          );
        })}
      ...
      <Text style={[styles.energyTrendHint, { color: colors.muted }]}>
        Акцент — дни с завершённой тренировкой.
      </Text>
    </Pressable>
  );
}
...
function NutritionProgress({
  ...
  const items = [
    ...
    { label: "Ж", value: macros.fat, goal: macroGoals.fat, color: "#2F5FC4" },
    {
      label: "У",
      value: macros.carbs,
  ...
  const caloriePercent = Math.min(
    100,
    Math.round((calories / Math.max(1, calorieGoal)) * 100),
  );
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        ...
        compact && compactStyles.nutritionProgress,
        {
          borderColor: colors.border,
          backgroundColor: colors.surface,
          opacity: pressed ? 0.68 : 1,
    ...
    >
      <View style={styles.nutritionHead}>
        <View>
          <Text style={[styles.nutritionKicker, { color: colors.primary }]}>
            ПИТАНИЕ · ЦЕЛЬ ДНЯ
          </Text>
          <Text
            style={[
              styles.nutritionCalories,
              compact && compactStyles.nutritionCalories,
              { color: colors.foreground },
            ]}
          >
            {calories}{" "}
            <Text style={styles.nutritionCaloriesSuffix}>
              / {calorieGoal} ККАЛ
            </Text>
          </Text>
        </View>
        <Text
          style={[
            styles.nutritionPercent,
            { color: calories > calorieGoal ? colors.error : colors.primary },
          ]}
        >
          {caloriePercent}%
        </Text>
      </View>
      <View style={[styles.nutritionTrack, { backgroundColor: colors.border }]}>
        <View
          style={[
            styles.nutritionFill,
            {
              ...
              backgroundColor:
                calories > calorieGoal ? colors.error : colors.primary,
            },
          ]}
        />
      </View>
        ...
        <View style={styles.macroProgressRow}>
          {items.map((item) => {
            const percent = Math.min(
              100,
              Math.round((item.value / Math.max(1, item.goal)) * 100),
            );
            return (
              <View key={item.label} style={styles.macroProgress}>
                <View style={styles.macroProgressHead}>
                  <Text
                    style={[styles.macroProgressLabel, { color: colors.muted }]}
                  >
                    {item.label}
                  </Text>
                  <Text
                    style={[
                      styles.macroProgressValue,
                      { color: colors.foreground },
                    ]}
                  >
                    {item.value}/{item.goal}г
                  </Text>
                </View>
                <View
                  style={[
...
function CompletedWorkoutChart({
  points,
  colors,
  compact,
}: {
  points: HomeWorkoutTrendPoint[];
  colors: any;
  compact: boolean;
}) {
  const width = 308;
  const height = compact ? 92 : 132;
  const baseline = compact ? 70 : 104;
  const paddingX = 12;
  ...
  const barWidth = points.length
    ? Math.max(
        ...
        Math.min(
          32,
          (width - paddingX * 2 - gap * Math.max(0, points.length - 1)) /
            points.length,
        ),
      )
    : 0;
  return points.length ? (
    <>
      <Svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        {points.map((point, index) => {
          ...
          const barHeight = Math.max(8, (point.volume / maxVolume) * 78);
          return (
            <Rect
              key={point.date}
              x={x}
              ...
              height={barHeight}
              fill={
                index === points.length - 1 ? REFERENCE_BLUE : colors.foreground
              }
              onPress={() =>
                router.push({
    ...
    <View style={styles.chartEmpty}>
      <Text style={[styles.chartEmptyText, { color: colors.muted }]}>
        Завершите тренировку, чтобы увидеть динамику объёма.
      </Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  nutritionKicker: { fontSize: 9, fontWeight: "900", letterSpacing: 0.8 },
  nutritionCalories: {
    fontSize: 26,
    fontWeight: "900",
    marginTop: 4,
  ...
  energyTrendHint: { fontSize: 9, fontWeight: "700", marginTop: 4 },
  dataRow: {
    flexDirection: "row",
    marginHorizontal: 12,
    marginTop: 14,
...
const compactStyles = StyleSheet.create({
  ...
  energyTrend: { paddingVertical: 10, marginTop: 9 },
  dataRow: { marginTop: 9 },
