...
export default function CalendarScreen() {
  ...
  const { programs, scheduled, completed, personalRecords, scheduleProgram, removeSchedule, startWorkout, deleteCompletedWorkout, notificationsEnabled, defaultWorkoutTime, defaultReminderMinutes } = useWorkoutStore();
  ...
  const selectedProgram = activeSchedule ? programs.find((item) => item.id === activeSchedule.programId) : undefined;
  const draftProgram = programs.find((item) => item.id === draftProgramId);
  const filteredPrograms = useMemo(() => { const query = programSearch.trim().toLocaleLowerCase("ru"); return query ? programs.filter((program) => `${program.name} ${program.description}`.toLocaleLowerCase("ru").includes(query)) : programs; }, [programSearch, programs]);
  ...
  const selectDate = (date: Date) => {
    ...
    const nextProgramId = entry?.programId ?? "";
    setSelectedDate(key);
    setProgramId(nextProgramId);
    setDraftProgramId(nextProgramId);
    setTime(entry?.time ?? defaultWorkoutTime);
    setReminderMinutes(entry?.reminderMinutes ?? defaultReminderMinutes);
  };
  const openPicker = () => {
    setDraftProgramId(activeSchedule?.programId ?? "");
    setProgramSearch("");
  ...
  const persistPlan = async (chosenProgramId: string) => {
    ...
    try {
      ...
      const scheduledTime = activeSchedule?.time ?? defaultWorkoutTime;
      const scheduledReminderMinutes = activeSchedule?.reminderMinutes ?? defaultReminderMinutes;
      const notificationId = notificationsEnabled ? await scheduleWorkoutReminder({ date: selectedDate, time: scheduledTime, reminderMinutes: scheduledReminderMinutes, programName: program.name }) : undefined;
      scheduleProgram(selectedDate, { programId: chosenProgramId, time: scheduledTime, reminderMinutes: scheduledReminderMinutes, notificationId });
  ...
  return <ScreenContainer edges={["top", "left", "right", "bottom"]} className="px-5" containerClassName="bg-background">
    <ScrollView contentContainerStyle={styles.content} scrollEnabled={!isDragging}>
      ...
      <View style={[styles.editor, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        ...
        </View> : activeSchedule && selectedProgram ? <><View style={[styles.selectedProgram, { backgroundColor: colors.background, borderColor: colors.border }]}><View style={{ flex: 1 }}><Text style={[styles.label, { color: colors.muted }]}>ЗАПЛАНИРОВАННАЯ ПРОГРАММА</Text><Text style={[styles.programName, { color: colors.foreground }]}>{selectedProgram.name}</Text><Text style={[styles.programDescription, { color: colors.muted }]}>{selectedProgram.exercises.length} упражнений · {selectedProgram.description}</Text></View><Pressable onPress={openPicker} style={[styles.change, { borderColor: colors.primary }]}><Text style={[styles.changeText, { color: colors.primary }]}>Изменить</Text></Pressable></View><Pressable onPress={deletePlan}><Text style={[styles.deleteText, { color: colors.error }]}>Удалить из расписания</Text></Pressable></> : <View style={[styles.emptyDateCard, { backgroundColor: colors.background, borderColor: colors.border }]}><IconSymbol name="calendar" size={23} color={colors.primary} /><Text style={[styles.emptyDateTitle, { color: colors.foreground }]}>Тренировка на эту дату не запланирована</Text><Text style={[styles.emptyDateHint, { color: colors.muted }]}>Выберите программу — параметры напоминания берутся из вкладки «Настройки».</Text><Pressable onPress={openPicker} style={[styles.save, { backgroundColor: colors.primary }]}><Text style={styles.saveText}>Запланировать тренировку</Text></Pressable></View>}
      </View>
    ...
    <Modal visible={programSearchOpen} transparent animationType="slide" onRequestClose={() => setProgramSearchOpen(false)}><View style={styles.backdrop}><View style={[styles.sheet, { backgroundColor: colors.background, paddingBottom: Math.max(20, insets.bottom + 12) }]}><View style={styles.sheetHeader}><View><Text style={[styles.sheetTitle, { color: colors.foreground }]}>Запланировать тренировку</Text><Text style={[styles.sheetSubtitle, { color: colors.muted }]}>Выберите программу на {readableDate(selectedDate)}</Text></View><Pressable onPress={() => setProgramSearchOpen(false)} style={[styles.close, { backgroundColor: colors.surface }]}><Text style={[styles.closeText, { color: colors.foreground }]}>×</Text></Pressable></View><View style={[styles.programSearch, { backgroundColor: colors.surface, borderColor: colors.border }]}><IconSymbol name="magnifyingglass" size={18} color={colors.muted} /><TextInput value={programSearch} onChangeText={setProgramSearch} placeholder="Поиск программы" placeholderTextColor={colors.muted} autoFocus style={[styles.programSearchInput, { color: colors.foreground }]} /></View><ScrollView contentContainerStyle={styles.programList}>{filteredPrograms.length ? filteredPrograms.map((program) => { const selected = draftProgramId === program.id; return <Pressable key={program.id} onPress={() => setDraftProgramId(program.id)} style={[styles.programOption, { backgroundColor: selected ? colors.primary + "20" : colors.surface, borderColor: selected ? colors.primary : colors.border }]}><View style={{ flex: 1 }}><Text style={[styles.optionName, { color: colors.foreground }]}>{program.name}</Text><Text style={[styles.optionDescription, { color: colors.muted }]}>{program.exercises.length} упражнений · {program.description}</Text></View><View style={[styles.radio, { borderColor: selected ? colors.primary : colors.muted }]}>{selected && <View style={[styles.radioDot, { backgroundColor: colors.primary }]} />}</View></Pressable>; }) : <Text style={[styles.emptyResults, { color: colors.muted }]}>По запросу программы не найдены.</Text>}</ScrollView><Pressable disabled={!draftProgram} onPress={() => draftProgram && void persistPlan(draftProgram.id)} style={[styles.confirm, { backgroundColor: colors.primary, opacity: draftProgram ? 1 : 0.5 }]}><Text style={styles.confirmText}>{draftProgram ? `Запланировать: ${draftProgram.name}` : "Выберите программу"}</Text></Pressable></View></View></Modal>
