...
export default function ServiceStatusScreen() {
  ...
  const { ready, programs, completed } = useWorkoutStore();
  ...
  const refresh = async () => { await checkPreview(); };
  const storageState: PreviewState = ready ? "online" : "checking";
  const reloadPreview = () => {
  ...
  return <ScreenContainer edges={["top", "left", "right", "bottom"]} className="px-5" containerClassName="bg-background"><ScrollView contentContainerStyle={styles.content}><View style={styles.header}><Pressable onPress={() => router.back()}><IconSymbol name="chevron.left" size={27} color={colors.foreground} /></Pressable><Text style={[styles.headerTitle, { color: colors.foreground }]}>Диагностика устройства</Text><View style={{ width: 27 }} /></View><View style={[styles.hero, { backgroundColor: colors.primary }]}><Text style={styles.heroEyebrow}>ЛОКАЛЬНАЯ АРХИТЕКТУРА</Text><Text style={styles.heroTitle}>Статус на устройстве</Text><Text style={styles.heroText}>Все данные тренировок хранятся и обрабатываются локально. Сервер и API не используются.</Text></View><Text style={[styles.section, { color: colors.foreground }]}>Текущее состояние</Text><View style={styles.cards}><StatusCard title="Локальное хранилище" description={ready ? `Готово: программ ${programs.length}, тренировок ${completed.length}.` : "Инициализируем данные на устройстве"} state={storageState} color={statusColor(storageState)} text={statusText(storageState)} foreground={colors.foreground} muted={colors.muted} surface={colors.surface} border={colors.border} /><StatusCard title="Интерфейс" description={Platform.OS === "web" ? "Открыт в текущем окне" : "Работает без подключения к API"} state={preview} color={statusColor(preview)} text={statusText(preview)} foreground={colors.foreground} muted={colors.muted} surface={colors.surface} border={colors.border} /></View><Text style={[styles.updated, { color: colors.muted }]}>{checkedAt ? `Последняя проверка: ${checkedAt.toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}` : "Статусы обновляются автоматически"}</Text><Pressable onPress={refresh} style={({ pressed }) => [styles.primary, { backgroundColor: colors.primary }, pressed && { opacity: 0.82 }]}><Text style={styles.primaryText}>Обновить статусы</Text></Pressable><Text style={[styles.section, { color: colors.foreground }]}>Быстрое восстановление</Text><View style={[styles.recovery, { backgroundColor: colors.surface, borderColor: colors.border }]}><Text style={[styles.recoveryTitle, { color: colors.foreground }]}>Перезагрузить экран</Text><Text style={[styles.recoveryText, { color: colors.muted }]}>Перезагружает текущий экран без потери локально сохранённых данных тренировки.</Text><Pressable onPress={reloadPreview} style={({ pressed }) => [styles.outline, { borderColor: colors.primary }, pressed && { opacity: 0.75 }]}><Text style={[styles.outlineText, { color: colors.primary }]}>Перезагрузить экран</Text></Pressable></View><View style={[styles.note, { backgroundColor: colors.warning + "12", borderColor: colors.warning + "45" }]}><Text style={[styles.noteTitle, { color: colors.foreground }]}>Конфиденциальность</Text><Text style={[styles.noteText, { color: colors.muted }]}>История тренировок, медиа и настройки остаются на этом устройстве. Для переноса используйте экспорт ZIP или CSV.</Text></View></ScrollView></ScreenContainer>;
}
