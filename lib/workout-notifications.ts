...
import {
  ...
  showNativeRestCountdown,
  type LockScreenHeartRateTarget,
  type NativeRestCompletionSound,
...
async function ensureLocalNotificationAccess(
  channelId: string,
  channelName: string,
 ...
 */
export async function scheduleRestTimerLockScreenNotification(
  restEndAt: number,
...
): Promise<RestTimerNotificationIds | undefined> {
  if (Platform.OS === "web") return undefined;
  const seconds = Math.max(1, Math.ceil((restEndAt - Date.now()) / 1000));
  if (
    !(await ensureLocalNotificationAccess(
      REST_TIMER_CHANNEL,
      "Таймер отдыха",
      [0, 90],
    ))
  ...
  const vibrationPattern = completionVibrationEnabled
    ? getCompletionVibrationPattern(completionVibrationPattern)
    : [];
  await ensureLocalNotificationAccess(
    completionChannelId,
    "Отдых завершён",
    vibrationPattern,
    completionSound !== "silent",
  );

  const nativeCountdownStarted = showNativeRestCountdown(
    restEndAt,
    target,
    completionSound,
    completionVolume,
  ...
  const activeId = nativeCountdownStarted
    ? undefined
  ...
  const completionId = nativeCountdownStarted
    ? undefined
    : await Notifications.scheduleNotificationAsync({
        content: {
        ...
        trigger: {
          ...
          seconds,
          channelId:
            Platform.OS === "android" ? completionChannelId : undefined,
        },
      });
  return { activeId, completionId };
}

export async function clearRestTimerLockScreenNotification(
  ids?: RestTimerNotificationIds,
) {
  clearNativeRestCountdown();
  if (Platform.OS === "web" || !ids) return;
  await Promise.all([
    ids.activeId
      ? Notifications.dismissNotificationAsync(ids.activeId).catch(
          () => undefined,
        )
      : undefined,
    ids.completionId
      ? Notifications.cancelScheduledNotificationAsync(ids.completionId).catch(
          () => undefined,
        )
      : undefined,
  ]);
...
export async function scheduleWorkoutReminder(input: {
  date: string;
  time: string;
  ...
  return Notifications.scheduleNotificationAsync({
    content: {
      ...
      body: `${input.programName} — через ${input.reminderMinutes} мин.`,
      sound: true,
